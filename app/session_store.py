...
def _json_safe(value):
    try:
        ...
        pass  # some values (e.g. a list/dict in a cell) don't support pd.isna — fall through

    if isinstance(value, datetime.timedelta):
        return str(value)
    if isinstance(value, (datetime.date, datetime.time)):
        # Covers datetime.date, datetime.datetime, AND pd.Timestamp (a
        # datetime.datetime subclass) with one check, plus bare
        # datetime.time cells. A column that mixes real dates with
        # non-date values (text, blanks, numbers) often stays object-dtype
        # with raw Python datetime.datetime objects instead of a uniform
        # pd.Timestamp column — that's the case that was missing before
        # and crashed with "Object of type datetime is not JSON serializable".
