...
# ---------------------------------------------------------------------------
# 3. Missing-value detection + smart fill
# ---------------------------------------------------------------------------

def _missing_mask(series: pd.Series) -> pd.Series:
    """NaN/None counts as missing; for text columns, blank/whitespace-only
    strings do too (a common source of 'invisible' missing data in Excel)."""
    mask = series.isna()
...
def missing_value_report(df: pd.DataFrame) -> dict:
    """Per-column missing-value stats with a suggested fill strategy for each."""
    total_rows = len(df)
    columns = []
    total_missing = 0
    ...
    for col in df.columns:
        ...
        is_numeric = pd.api.types.is_numeric_dtype(series) or (
            series.dtype == object
            and pd.to_numeric(series, errors="coerce").notna().mean() > 0.7
        )
        if missing_count == 0:
            suggested = None
        elif is_numeric:
            suggested = "median"  # robust to outliers, better default than mean
        else:
            suggested = "mode"
        ...
        columns.append({
            ...
            "missing_pct": round(missing_count / total_rows * 100, 1) if total_rows else 0.0,
            "suggested_method": suggested,
        })

    columns.sort(key=lambda c: -c["missing_count"])
...
_VALID_FILL_METHODS = {"mean", "median", "mode", "constant", "ffill", "bfill", "drop_rows"}


def fill_missing(df: pd.DataFrame, strategies: dict) -> pd.DataFrame:
    """
    strategies: {column_name: {"method": "mean"|"median"|"mode"|"constant"|"ffill"|"bfill"|"drop_rows", "value": <for constant>}}
    Applied one column at a time, in the order given, on a copy of df.
    """
    result = df.copy()
    ...
    for col, spec in strategies.items():
        ...
        if method not in _VALID_FILL_METHODS:
            continue
        ...
        if method == "drop_rows":
            ...
            result[col] = series.mask(mask).ffill()
            continue
        elif method == "bfill":
            result[col] = series.mask(mask).bfill()
            continue
        elif method == "mean":
        ...
        if pd.isna(fill_value):
            continue  # nothing sensible to fill with (e.g. all-missing numeric column)
        result.loc[mask, col] = fill_value
